...
def main():
    ...
    parser.add_argument("--mode", type=str, default="v1.0", choices=["v1.0", "v3.0", "v4.0"],
                        help="因子模式: v1.0(三因子) 或 v3.0(五因子) 或 v4.0(因子库选优5因子)")
    ...
    try:
        ...
        if len(volume) >= 20:
            ...
            mean_10 = np.mean(vol_delta[-10:])
            mean_20 = np.mean(vol_delta[-20:])
            std_20 = np.std(vol_delta[-20:])
            accel = (mean_10 - mean_20) / (std_20 + 1e-8)
            accel_hist = [(np.mean(vol_delta[max(0,i-10):i]) - np.mean(vol_delta[max(0,i-20):i])) / (np.std(vol_delta[max(0,i-20):i]) + 1e-8) for i in range(20, len(vol_delta)+1)]
            factor4 = sum(1 for a in accel_hist if a < accel) / len(accel_hist) if accel_hist else 0.5
        else:
            factor4 = 0
